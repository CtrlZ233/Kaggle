...
def fitness(batch_size_flaot, pow_lr):
    if batch_size_flaot < 0.3:
        batch_size = 10
    elif batch_size_flaot <0.6:
        batch_size = 20
    else:
        batch_size = 50
    ...
    net = simpleNet()
    optim = torch.optim.SGD(net.parameters(), lr=pow_lr/10000, weight_decay=0.0001)
    ...
    for epoch in range(epoch_num):
        ...
        for batch_image, batch_label in train_loader:
            ...
            loss.backward()
            optim.step()
            print("train:epoch:{}, data_index:{}/4000, loss:{:.4f}".format(epoch, index, loss))
            if index > 4000:
                break
        print("================test================")
        right_count = 0

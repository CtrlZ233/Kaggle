...
if __name__ == "__main__":
    ...
    if exist:
        level_1_Net_1.load_state_dict(torch.load("./model/level_1_model/1/Net_1_33.6446.pth"))
        level_1_Net_2.load_state_dict(torch.load("./model/level_1_model/2/Net_2_33.6446.pth"))
    ...
    for epoch in range(epoch_num):
        ...
        for batch_image, batch_label in full_train_loader:
            ...
            train_loss1.append(float(loss.data)/(batch_image.shape[0]*30))
            loss.backward()
            optim.step()
        ...
        for batch_image, batch_label in half_train_loader:
            ...
            prediction1 = level_1_Net_1.forward1(batch_image.view(batch_image.shape[0], 1, 96, 96))
            prediction2 = level_1_Net_2.forward2(batch_image.view(batch_image.shape[0], 1, 96, 96))
            prediction = prediction2 + prediction1[:,[0, 1, 2, 3, 20, 21, 28, 29]]
            # print(prediction.shape)
            # print(batch_label.shape)
            optim.zero_grad()
            loss = criterion(prediction, batch_label)
            train_loss1.append(float(loss.data)/(batch_image.shape[0]*8))
            loss.backward()
            optim.step()
        ...
        for batch_image, batch_label in validate_loader:
            ...
            loss = criterion(prediction, batch_label)
            loss_array.append(loss.sum().data)
        loss_meam = sum(loss_array)/validate_data.len
        test_loss.append(float(loss_meam/30))
        ...
        viz.line(Y=np.array([float(loss_meam/30)]), X=np.array([epoch]) ,update='append',win=win1)
        viz.line(Y=np.array([float(mean(train_loss1))]), X=np.array([epoch]), update='append', win=win2)
